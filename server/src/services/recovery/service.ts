...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function resolveContinuationWaitingOnReview(issue: typeof issues.$inferSelect) {
    ...
    const excludeIds = new Set(childrenCreatingParentEdge.map((r) => r.issueId));
    const safeChildren = openChildren.filter((c) => !excludeIds.has(c.id));
    // Drop pre-existing blockers that would create a parent<->child cycle too
    // (the same AWAITING-YOU shape can appear via existingBlockers when the
    // AWAITING-YOU edge was added before this recovery tick).
    const safeExistingBlockers = existingBlockers.filter((row) => !excludeIds.has(row.id));
    const blockedByIssueIds = [...new Set([...safeExistingBlockers.map((row) => row.id), ...safeChildren.map((row) => row.id)])];
    if (blockedByIssueIds.length === 0) return null;

    const updated = await issuesSvc.update(issue.id, { status: "blocked", blockedByIssueIds });
    if (!updated) return null;

    const waitingOn = formatIssueLinksForComment([...safeChildren, ...safeExistingBlockers]);
    await issuesSvc.addComment(
      issue.id,
